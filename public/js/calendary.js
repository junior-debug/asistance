...
function monthsSelect() {
  for (let i = 1; i < month + 1; i++) {
    $("#selectMonth").append(
      `<option value="${months[i].toLowerCase()}">${months[i]}</option>`
    );
  }
}
monthsSelect();
...
function threeDays(days, dayPosition, totalDaysMonth, today, selectedMonth, estatusCambios, id, notHired, dayhired, fecha_egreso) {
  if (days.length == 3) {
    for (let i = 0; i < totalDaysMonth; i++) {
      if (
...
function fourHolidays(rotation, dayPosition, totalDaysMonth, today, selectedMonth, estatusCambios, id, notHired, dayhired, fecha_egreso) {
  let days = [];
  ...
  for (let i = 0; i < res.length; i++) {
    ...
    if (
      ...
      if (userChanges == false) {
        ...
        for (let x = 0; x < $(".dataDay").length; x++) {
          $(`.tableAsistance${userChanges}`).append(
            `<td class ="day${userChanges}" style="border: 1px solid rgba(128, 128, 128, 0.342);text-align: center;"></td>`
          );
        }
    ...
    statusCode: {
      200: function (data) {
        for (let y = 0; y < data.length; y++) {
          if (
            data[y].nomina_cliente != lastPayroll &&
            ...
            const newObject = [
              {
                ...
                finalizacion_segundo_contrato:
                  data[y].finalizacion_segundo_contrato,
                finalizacion_tercer_contrato:
                  data[y].finalizacion_tercer_contrato,
                cargo: data[y].cargo,
...
function buildJson(res, data, selectedMonth, firstDayWeek, totalDaysMonth) {
  for (let i = 0; i < res.length; i++) {
    if (res[i].estatus_cambios == 1) {
      for (let y = 0; y < data.length; y++) {
        if (
          selectedMonth == data[y].fecha.slice(5, 7) &&
          data[y].antigua_nomina != "" &&
          data[y].nomina == "" &&
          data[y].cedula == res[i].cedula
        ) {
    ...
    statusCode: {
      200: function (data) {
        if (allPayrolls == true) {
          ...
          for (let i = 0; i < data.length; i++) {
            if (
              (data[i].nomina == payRolls &&
                data[i].fecha.slice(5, 7) == selectedMonth) ||
              (data[i].antigua_nomina == payRolls &&
                data[i].fecha.slice(5, 7) == selectedMonth)
            ) {
              gif(30000);
              validationData(
